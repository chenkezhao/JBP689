 ...
 * <ul>
 * <li>普通权限：</li>
 * <li>如果一个应用程序在其清单声明，它需要一个正常的权限时，系统会自动授予权限在安装时的应用。系统不提示用户授予权限正常，并且用户无法撤消这些权限。</li>
 * <li>由于API级别23，下列权限划分为PROTECTION_NORMAL：</li>
 ...
 * <ul>
 *  <li>以下权限需要在Android6.0上被允许，才能正常工作，开发者需要在调用有米的任何代码之前，提前让用户允许权限</li>
 *  <li>必须申请的权限</li>
 *  <li>》》》》》》》》》》》》》》友盟必须权限(需用户同意)：</li>
 * <ul>
 ...
 * </ul>
 *  <li>》》》》》》》》》》》》》》腾讯广点通必须权限(需用户同意)：</li>
 * <ul>
 * <li>READ_PHONE_STATE</li>
 * <li>ACCESS_COARSE_LOCATION</li>
 * <li>ACCESS_FINE_LOCATION——如果需要精确定位的话请加上此权限</li>
...
//			new PermissionModel("电话", Manifest.permission.READ_PHONE_STATE, "我们需要读取手机信息的权限来标识您的身份", READ_PHONE_STATE_CODE)
			//腾讯广点通必须权限
			//new PermissionModel("粗略位置", Manifest.permission.ACCESS_COARSE_LOCATION, "我们需要您允许我们读取你的粗略位置，以方便我们能更好优化软件，符合大众喜爱的一款软件",ACCESS_COARSE_LOCATION_CODE)
			//其它
			new PermissionModel("存储空间", Manifest.permission.WRITE_EXTERNAL_STORAGE, "我们需要您允许我们读写你的存储卡，以方便我们临时保存一些数据",WRITE_EXTERNAL_STORAGE_CODE)
	};
